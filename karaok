...
#define SSID           "iPhone X"                   // cannot be longer than 32 characters!
#define PASSWORD       "19990513"
...
#define DEVICEID   "20492989"
#define PROJECTID "108406"
#define HOST_PORT   (80)
String apiKey="SavcJlOJmMzjRLfOUXI0K7CkQ7w=";
char buf[10];
...
#include <SoftwareSerial.h>
SoftwareSerial yourSerial(2, 3); /* RX:D3, TX:D2 */
ESP8266 wifi(yourSerial);
...
void updateSensorData() {
  if (wifi.createTCP(HOST_NAME, HOST_PORT)) { //建立TCP连接，如果失败，不能发送该数据
    ...
    jsonToSend="{\"MusicNumber\":";
    dtostrf(music_num,1,2,buf);
    jsonToSend+="\""+String(buf)+"\"";
    jsonToSend+=",\"MusicVolume\":";
    dtostrf(music_vol,1,2,buf);
    jsonToSend+="\""+String(buf)+"\"";
    jsonToSend+="}";
